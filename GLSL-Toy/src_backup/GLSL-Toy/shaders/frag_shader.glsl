...
vec2 uResolution = vec2(640.f, 480.f);
...
void main()
{
    ...
    uv_lb            += uv_cc                           
                        *
                        sin(uTime)                      // making the whole animation periodic
                        *
                        abs(sin(len * 9. - uTime))      // adding another period (with offset)
                        /
    ...
    vec3 color        = vec3(col, col + .07, col + .07 + .07) / len; 
    
    // output
    outColor=vec4(color, uTime);
}